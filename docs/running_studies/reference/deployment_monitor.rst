...
- **Experimenter**: The name of the experimenter who created the experiment. We recommend setting this in your ``.dallingerconfig`` file, see :doc:`Slack integration </running_studies/reference/setting_up_slack>`.
...
- **Server**: Indicates where the experiment is hosted (``local``, ``ssh``, or ``heroku``). Clicking shows the server's latest resource usage (CPU, memory, disk, etc.). Warnings and failures are flagged if resource limits are exceeded. Slack notifications are triggered if :doc:`configured </running_studies/reference/setting_up_slack>`.
- **Errors**: Displays the number of errors that occurred. Click to view error types and counts. Slack alerts are sent if :doc:`Slack integration </running_studies/reference/setting_up_slack>` is enabled, with direct links to error details and stack traces.
