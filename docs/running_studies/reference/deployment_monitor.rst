...
- **Server**: Indicates where the experiment is hosted (``local`` or ``ssh``). Clicking shows the server's latest resource usage (CPU, memory, disk, etc.). Warnings and failures are flagged if resource limits are exceeded. Slack notifications are triggered if :doc:`configured </running_studies/reference/setting_up_slack>`.
