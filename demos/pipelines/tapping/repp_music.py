...
from .repp_utils import NumpySerializer, TapTrial
...
def get_music_stimuli_loader(
    stimuli: PathLike, bot_responses: Optional[PathLike] = None
):
    """
    ...
    def deferred():
        ...
        if not len(audios) > 0:
            raise FileNotFoundError(
                f"No audio files found in {stimuli}. Note: only .wav files are supported for now."
            )
        ...
        if bot_responses is not None:
            ...
            bot_audios = [bot_responses / f"{name}.wav" for name in names]
